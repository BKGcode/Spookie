...
public class TaskListPanel : MonoBehaviour
{
    ...
    private void RefreshTaskList()
    {
        ...
        foreach (TaskData task in tasksForThisPanel)
        {
         ...
         Debug.Log($"[{gameObject.name}] Refreshed. Displaying {spawnedItemsMap.Count} tasks.");
    }
    ...
    private void HandleTaskFeedback(string taskId, string messageKey)
    {
        ...
        if (spawnedItemsMap.TryGetValue(taskId, out TaskItemUI targetItemUI))
        {
            ...
            {
                 // Item might have been destroyed between event firing and handling, remove dangling key
                 spawnedItemsMap.Remove(taskId);
            }
        }
         // If the taskId is not in the map, it means the feedback is not for an item currently in this panel
    }
}

// --- Summary Block ---
// ScriptRole: Manages the UI list displaying tasks in 'Pending' or 'Stopped' state. Instantiates and updates TaskItemUI prefabs. Handles feedback events for relevant tasks.
